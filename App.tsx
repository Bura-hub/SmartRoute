...
const App: React.FC = () => {
  ...
  const handleCalculateInstant = async () => {
    setIsStepMode(false);
    setAutoPlay(false);
    setStepState(null);
  ...
  const handleStartStepMode = () => {
    ...
    // Initialize Iterator
    if (algorithm === AlgorithmType.DIJKSTRA) {
      iteratorRef.current = dijkstraStepGenerator(graphData, startNode, endNode, weightType, transportMode);
    } else {
      iteratorRef.current = bellmanFordStepGenerator(graphData, startNode, endNode, weightType, transportMode);
    }
  ...
  return (
    <div className="relative w-screen h-screen bg-transparent font-sans overflow-hidden">
      ...
      <Graph3D 
        ...
        stepState={stepState} // Pass step state for visualization
      />
    </div>
  );
};
